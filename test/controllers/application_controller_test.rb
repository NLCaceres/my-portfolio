...
class ApplicationControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'Turnstile human verification delivers a particular response' do
    ...
    post send_email_url, headers: accept_header, params: { email: 'foo@example.com', message: 'Barfoo', cfToken: '1' }
  ...
  test 'should send email if verified by Turnstile' do
    #* Following dummy secret key always succeeds so send successful response w/ email sent out
    ENV['TURNSTILE_SECRET_KEY'] = '1x0000000000000000000000000000000AA'
    post send_email_url, headers: accept_header, params: { email: 'foo@example.com', message: 'Barfoo', cfToken: '1' }
    successful_email_response = @response.parsed_body
    assert_equal 'Successfully sent your email!', successful_email_response['message']

    #* Following always fails so should get a fail response w/ no email sent
    ENV['TURNSTILE_SECRET_KEY'] = '2x0000000000000000000000000000000AA'
    post send_email_url, headers: accept_header, params: { email: 'foo@example.com', message: 'Barfoo', cfToken: '1' }
    failed_email_response = @response.parsed_body
