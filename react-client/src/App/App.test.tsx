...
describe("renders the whole app", () => {
  ...
  beforeEach(() => {
    ...
    ApiMock = jest.spyOn(GetPostList, "default").mockResolvedValue({ majorProjects: [majProject], minorProjects: [minProject] });
  })
  ...
  describe("controls the opening of an app-wide alert element", () => {    
    test("showing a danger alert if contact-me submit button fires without permission", async () => {
      ...
      jest.spyOn(CommonAPI, "SendEmail").mockResolvedValue("123"); //* Invalid response so Turnstile thinks user is a computer
    ...
    test("showing a success alert if contact-me submit button fires when expected", async () => {
      ...
      jest.spyOn(CommonAPI, "SendEmail").mockResolvedValue( //* Must return a valid response for Turnstile to provide a success response
